...
AddEventHandler('BanSql:ICheatClient', function(reason)
	...
	if _source then
		...
		if name then
			...
			AddBan(_source, licenseid, playerip, name, 'Anti-Cheat AeroDefence', 0, reason, 1)
			DropPlayer(_source, ('Vous êtes banni de Velocity\nRaison : %s\nTemps Restant : Permanent\nAuteur : Anti-Cheat Velocity'):format(reason))
		end
	else
...
AddEventHandler('BanSql:ICheatServer', function(target, reason)
	...
	if target then
		...
		if name then
			...
			AddBan(target, licenseid, playerip, name, 'Anti-Cheat Velocity', 0, reason, 1)
			DropPlayer(target, ('Vous êtes banni de Velocity\nRaison : %s\nTemps Restant : Permanent\nAuteur : Anti-Cheat Velocity'):format(reason))
		end
	else
		print('BanSql Error : Anti-Cheat California have received invalid id.')
	end
